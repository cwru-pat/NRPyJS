...
//The only way to apply sommerfeld is via RK-MoL, new way
//#include "MoL_step_forward_in_time.h"
...
//We will change this to getter functions to grab the initial data from Leos initial conditions
    ...
    fprintf(stderr,"Here is part of y_n_gfs: %f\n",in_gfs[12]);
    ID_BSSN__ALL_BUT_LAMBDAs(params,xx,SF_in, in_gfs);
    fprintf(stderr,"2Here is part of y_n_gfs: %f\n",in_gfs[12]);
    apply_bcs_curvilinear(params, bcstruct, NUM_EVOL_GFS, evol_gf_parity, in_gfs);
    fprintf(stderr,"3Here is part of y_n_gfs: %f\n",in_gfs[12]);
    //MoL_step_forward_in_time(,in_gfs)
    enforce_detgammahat_constraint(rfmstruct, params,                   in_gfs);
    fprintf(stderr,"4Here is part of y_n_gfs: %f\n",in_gfs[12]);
    ID_BSSN_lambdas(params, xx, in_gfs);
    fprintf(stderr,"5Here is part of y_n_gfs: %f\n",in_gfs[12]);
    apply_bcs_curvilinear(params, bcstruct, NUM_EVOL_GFS, evol_gf_parity, in_gfs);
    fprintf(stderr,"6Here is part of y_n_gfs: %f\n",in_gfs[12]);
    enforce_detgammahat_constraint(rfmstruct, params,                   in_gfs);
    fprintf(stderr,"7Here is part of y_n_gfs: %f\n",in_gfs[12]);
...
#include "rfm_files/rfm_struct__malloc.h"
    ...
    const REAL dtt = 0.0;
    // COPYING AND PASTING MoL_step_forward_in_time.h with dt = 0, 
    // which hopefully just applies the boundary conditions.
#include "MoL_step_forward_in_time.h"
