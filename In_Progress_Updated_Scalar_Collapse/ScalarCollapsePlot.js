...
async function graphSlice(){
    ...
    if(isRunning){
        ...
        if(n%20 == 0) _stepForward(true);
        else _stepForward(false);
...
function runSim(){
    ...
    _setDim(document.getElementById("res1").value,8,2);
    
    makeInitials(parseFloat(document.getElementById("phi0").value),parseFloat(document.getElementById("r0").value)
                 ,parseFloat(document.getElementById("sigma").value));
    //makeInitials()
    console.log(r[0]);
    console.log(sf[29999]);
    console.log(psi4[29999]);
    console.log(alpha[29999]);
    
    for(let i=0; i<r.length; i++){
        _setInitR(r[i],i);
        _setInitSf(sf[i],i);
        _setInitPsi4(psi4[i],i);
        _setInitAlpha(alpha[i],i);    
    }
    if(verbose) console.log("Running");
    _runsim();
    if(verbose) console.log("Done");
    
    t_final = parseInt(document.getElementById("tfin").value);
    dt = _getdt();
    N_finalp = t_final/dt + 0.5;
    _setNFinal(N_finalp);
    N_final = _getNFinal();
